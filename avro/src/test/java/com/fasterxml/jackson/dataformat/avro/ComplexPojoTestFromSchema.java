...
public class ComplexPojoTestFromSchema extends AvroTestBase {
	...
	protected final static String JVM_SERIALIZERS_SCHEMA_STR = "{\"type\":\"record\",\"name\":\"MediaContent\",\n"
			+ "\"namespace\":\"serializers.avro.media\"," + "\"fields\":[\n"
			...
			+ "{\"name\":\"media\",\"type\":{" + "  \"type\":\"record\",\"name\":\"Media\",\n"
	...
	public ComplexPojoTestFromSchema() {
		avroMapper = new ObjectMapper(new AvroFactory());
		avroMapper.enable(SerializationFeature.WRITE_ENUMS_USING_INDEX);
		itemSchema = itemSchema();
		itemReader = avroMapper.readerFor(MediaItem.class).with(itemSchema);
		itemWriter = avroMapper.writerFor(MediaItem.class).with(itemSchema);
	}
	...
	protected GenericRecord itemToRecord(MediaItem item) throws IOException {
		...
		return r.read(null, DECODER_FACTORY.binaryDecoder(new ByteArrayInputStream(avro), null));
	}

	public void testRoundTrip() throws Exception {
		final MediaItem item = buildItem();
		byte[] avro = itemToBytes(item);
		MediaItem readItem = itemReader.readValue(avro);
